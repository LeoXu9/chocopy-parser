...
￼                 otherwise.
    ...
    def parse_function(self) -> Operation:
        ...
        # Function parameters
        parameters: List[Operation] = self.parse_typed_var_list()
        ...
        while True:
            if self.check(TokenKind.GLOBAL):
                defs_and_decls.append(self.parse_global_decl())
            elif self.check(TokenKind.NONLOCAL):
                defs_and_decls.append(self.parse_nonlocal_decl())
            elif self.check([TokenKind.IDENTIFIER, TokenKind.COLON]):
                defs_and_decls.append(self.parse_var_def())
            else:
                break
